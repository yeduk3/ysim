...
#define METAL_SYSTEM
    ...
    auto render = [&]() {
        ...
        // 하단 진행 바 높이/여백. 좌우 사이드 패널은 이 바 위쪽 영역만
        // 차지하도록 위아래 크기가 줄어든다.
        const float barH    = ImGui::GetFrameHeight()
                            + ImGui::GetStyle().WindowPadding.y * 2.0f + 6.0f;
        const float barGap  = 8.0f;
        const float availH  = vp->WorkSize.y - menuH - barH - barGap;
        const float panelH  = availH - 20.0f;
        const float yCenter = vp->WorkPos.y + menuH + availH * 0.5f;
