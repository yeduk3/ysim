...
## CM-011 — Intermediate `commitAndWait` inside the per-substep loop changes when CPU/GPU reads observe post-integrator state, BUT this is the correct standard sim order

- Where: `Simulator::update`'s per-substep loop (`src/main.cpp` ~line 4763). The substep loop runs: refit → enlarge → detect (commits) → narrow (commits) → xPrev memcpy → integrator (no commit). With OLD CPU refit, refit + enlarge read `positions.ptr` BEFORE the next commit boundary; positions.ptr at that moment reflects state from BEFORE the prior substep's integrator finished (the integrator dispatch sat in the encoder uncommitted until the next detect/queryEnd commit on the NEXT substep). Effectively: OLD CPU refit operates on positions 2 substeps stale. With NEW GPU refit (D-029), the refit's own `commitAndWait` flushes the prior substep's integrator dispatch before refit's leaf-build kernel reads positions; refit therefore sees positions 1 substep stale — i.e., **the standard "BVH reflects post-prior-substep-integrator state"** order.
- Low-level cause: deferred-commit timing. Same kernels produce different sim trajectories depending on WHEN their CPU/GPU reads of `positions.ptr` happen relative to the integrator's commit boundary.
- High-level cause: the OLD per-substep loop's correctness for BDD-010 depended on a non-documented invariant ("BVH lags 2 substeps") that was an artifact of CPU/GPU async via deferred commit. NEW GPU refit makes the lag standard (1 substep), which is the typical simulation order: detect collisions at current state → resolve → integrate. The BDD-010 test's `lastSubstep`-iteration assertion was incidentally satisfied by OLD's deeper lag (cloth-ground contact stayed visible across all substeps); with NEW's standard lag, contact response fully resolves cloth-ground overlap by substep ~2 and the last substep is contact-free.
- Fix direction (RESOLVED — D-029 ships GPU refit; CM-011 is forensic, not active risk): the right honest mechanization is `cumulativeNarrowCollisions > 0` (cumulative across substeps), which with `enableSelfCollisions = false` (Simulator default) reliably means "at least one inter-object contact pair fired during the frame". The `lastSubstep`-iteration check was redundant once cumulativeNarrowCollisions exists and self-collisions are filtered. Block 13's BDD-010 positive case was simplified accordingly. Future GPU ports of substep-loop work (e.g., GPU `enlargeTrajectory`) should NOT re-introduce `lastSubstep`-style assertions — the cumulative pattern is the canonical shape.
- First seen: 2026-05-11   Last seen: 2026-05-11 (resolved same day; kept as forensic record).


## CM-012 — Utility helper calls `exit(1)` on failure path, defeating harness SKIP semantics
- File / function: `include/program.hpp` — `Program::printLog` and `Program::linkShader` historically called `exit(1)` on shader link failure, propagating the unilateral exit out of every `Program::loadShader` overload.
- Low-level cause: a "print the error log" helper made a process-lifetime decision the caller wasn't ready for. Harness code that documented a `programID == 0` SKIP semantic (e.g., Block 25 in `runSelfTest`) never got to observe the failure because the process was already gone.
- High-level cause: utility helpers should communicate state (return a value, set a flag) and let callers decide whether the failure is fatal. When a helper decides process lifetime unilaterally, it forces every caller — including test harnesses, in-process embeddings, future replay tools — to accept that decision, which is rarely what's wanted at the architectural level. The trap is especially insidious for helpers tucked inside long call chains where the exit point isn't visible from the call site.
- Fix direction (RESOLVED — D-034): print-only helpers print, return-bool helpers return, and any `cleanUp() + return` discipline happens at the level that owns the state machine. Production callers that want loud failure add their own `if (!programID) std::exit(1)` at the call site (single-site defensive check in `src/main.cpp:8460` for the main render shader). Harness callers consult `programID == 0` for SKIP. The contract is "loader produces state; caller decides fatality."
- Audit pattern: when adding a new utility helper, ask "does this function decide process lifetime?" If yes, the helper's signature should make that decision visible (e.g., `[[noreturn]] void crashOnFailure(...)` if exit is unconditional, OR `bool tryLink(...)` if the helper just communicates). Implicit `exit(1)` inside an innocuously-named helper is a code smell.
- First seen: 2026-05-12 (Estimator turn 27 WARNING).   Resolved: 2026-05-13 (D-034 ships the contract refactor + Block 27 mechanization).
