...
| BDD-005     | Edit OpenPBR material parameter                   | `src/main.cpp::runSelfTest` Block 20 — four clauses PASS (`BDD-005 / setMaterial writes all 5 fields to mesh.material`, `…/ material survives one sim step` (BDD-103 backend-boundary), `…/ material round-trips through saveScene/loadScene`, `…/ material survives Scene::pack rebuild` (D-025 sister mechanization)). Closed by D-027 (`Simulator::setMaterial` mutator + `pendingMaterials` write-back). Renderer-side clause from TESTS.md#BDD-005 ("preview render reflects the lower roughness") is parked under documented-gap convention pending a PBR-preview-shader slice. Inspector wiring at `src/mesh_inspector_gui.cpp` renders 2 ColorEdit3 + 3 SliderFloat (0..1) widgets, each firing `Simulator::setMaterial` on change. | warning |
