...
| BDD-007     | Cloth drapes onto a rigid surface                 | `src/main.cpp::runSelfTest` Block 6 — three of four clauses PASS (`BDD-007 / cloth meanY decreases over time`, `BDD-007 / contact constraints fire on broad/narrow phase`, `BDD-007 / total energy stays bounded`); `BDD-007 / no cloth vertex tunnels through ground` FAILs (CM-005, snapshot narrow-phase misses fast-moving thin cloth crossing static surface; needs CCD slice). The `enlargeTrajectory(system.subh)` fix in this slice enabled the broad-phase detection clause. | warning |
