...
- **BDD-018-BEHAVIOR-TAG-PARKED** (turn-28 WARNING, 2026-05-13). BDD-018 matrix row marked `pass`, but the "if the behavior changed, the next sim step dispatches through the new behavior" clause is parked behind BDD-006 / Q2 (in-place behavior switching, blocked on cloth-UX-surface decision). The Estimator should not re-flag this every turn. Retires when BDD-006 ships (Q2 resolution + behavior-tag inspector wiring).
