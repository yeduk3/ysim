...
- **BDD-018-BEHAVIOR-TAG-PARKED** — **RETIRED by D-039, 2026-05-14.** B-3 wired the Rigid behavior tag through `Simulator::update` via `ensureRigidBackendBody` + Δpos delta-loop; inspector Float→Rigid switch now dispatches through the new behavior on the next sim step. Historical record in `docs/DECISIONS.md` D-039.
- **BDD-006-RIGID-DISPATCH-PARKED** — **RETIRED by D-039, 2026-05-14.** B-3's `Simulator::update` C++ delta-loop integrates Rigid bodies via the Euler backend; `applyEnvironmentForces` now skips Rigid (zeros externalForces, mirroring Float); persistence works via `Simulator::initialize`'s post-pack sweep. Historical record in `docs/DECISIONS.md` D-039.
...
- **RIGID-BACKEND-PORTABILITY** (D-037, 2026-05-14; reinforced by D-038, 2026-05-14; **reinforced by D-040, 2026-05-14**). Any change to the `RigidPhysicsBackend` contract surface (`include/RigidPhysicsTypes.hpp` POD types OR the 12-method signature on backend classes — `initialize / shutdown / addBody / removeBody / step / getPosition / getRotation / getLinearVelocity / getAngularVelocity / applyForce / applyImpulse / setLinearVelocity / setAngularVelocity / setGravity / backendName`) MUST update every backend implementation in the same commit. **As of D-040: three backends are live — `NullRigidPhysicsBackend` (`include/NullRigidPhysicsBackend.hpp`) + `EulerRigidPhysicsBackend` (`include/EulerRigidPhysicsBackend.hpp`) + `BulletRigidPhysicsBackend` (`include/BulletRigidPhysicsBackend.hpp` + `src/BulletRigidPhysicsBackend.cpp`). `Simulator::rigid_` is hard-coded `BulletRigidPhysicsBackend` per D-040; the swap was a one-line type change as the contract promised.** Future Jolt / custom backends plug in by satisfying the contract. Retires only if the contract surface is collapsed.
