...
- **RIGID-BACKEND-PORTABILITY** (D-037, 2026-05-14; reinforced by D-038, 2026-05-14). Any change to the `RigidPhysicsBackend` contract surface (`include/RigidPhysicsTypes.hpp` POD types OR the 12-method signature on backend classes — `initialize / shutdown / addBody / removeBody / step / getPosition / getRotation / getLinearVelocity / getAngularVelocity / applyForce / applyImpulse / setLinearVelocity / setAngularVelocity / setGravity / backendName`) MUST update every backend implementation in the same commit. **As of B-2′: two backends are live — `NullRigidPhysicsBackend` (`include/NullRigidPhysicsBackend.hpp`) AND `EulerRigidPhysicsBackend` (`include/EulerRigidPhysicsBackend.hpp`). Bullet (`docs/design/rigid_physics_backend.md` §B-2 — canonical implementation) deferred to a future slice (B-2.1) until vendor permission is granted; will become the third backend at that point.** Future Jolt / custom backends plug in by satisfying the contract. Retires only if the contract surface is collapsed.
